...
function ProductDetail() {
    ...
    useEffect(() => {
        ...
        axios.get(url)
            .then((res) => {
                if (res.data.product) {
                    setProduct(res.data.product);
                    // Fetch seller info immediately
                    handleContact(res.data.product.addedBy);
                }
            })
            .catch((err) => {
                alert('Server Err.');
    ...
    // New function to handle chat navigation
    const handleChatNow = () => {
        if (!product || !user) return;
        navigate(`/chat/${product._id}/${user._id}`); // Goes to chat page
    };
                        ...
                        <div>
                            ...
                            {user && (
                                <div className="seller-info">
                                    {user.username && <h4>Seller: {user.username}</h4>}
                                    {user.mobile && <p>Contact: {user.mobile}</p>}
